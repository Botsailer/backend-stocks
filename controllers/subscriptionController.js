...
const createOrFetchCustomer = async (razorpay, user) => {
  try {
    ...
    if (!user || !user.email) {
      logger.error("Invalid user data for emandate subscription", {
        userId: user?._id?.toString() || 'unknown',
        userExists: !!user,
        emailExists: !!(user && user.email)
      });
      throw new Error("Valid email is required for emandate subscription");
    }
    
    if (!user.email.includes('@')) {
      logger.error("Invalid email format for emandate subscription", {
        userId: user._id?.toString(),
        email: user.email
      });
      throw new Error("Valid email address format is required for emandate subscription");
    }
    ...
    try {
      ...
      if (!user || !user.email) {
        logger.error("Cannot create fallback customer: missing user or email", {
          userExists: !!user,
          emailExists: !!(user && user.email)
        });
        throw new Error("Valid email is required for customer creation");
      }
...
exports.createEmandate = async (req, res) => {
  ...
  try {
    ...
    if (!req.user || !req.user.email) {
      logger.error("EMandate creation failed: Missing user data", {
        userId: userId.toString(),
        userExists: !!req.user,
        emailExists: !!(req.user && req.user.email),
        timestamp: new Date().toISOString()
      });
      return res.status(400).json({ 
        success: false, 
        error: "Your account is missing required information. Please update your profile with a valid email address.",
...
exports.verifyPayment = async (req, res) => {
  ...
  try {
    ...
    // Prevent duplicate processing but provide more helpful response
    const existingPayment = await PaymentHistory.findOne({ paymentId });
    if (existingPayment) {
      await session.abortTransaction();
      
      // Get information about the existing subscription
      const existingSubscription = await Subscription.findOne({ paymentId }).populate({
        path: 'productId',
        select: 'name portfolioName'
      });
      
      // Look for any Telegram invites that might have been generated previously
      let telegramInvites = [];
      if (existingSubscription && existingSubscription.invite_link_url) {
        telegramInvites.push({
          productId: existingSubscription.productId?._id || existingSubscription.productId,
          product_name: existingSubscription.productId?.name || 
                        existingSubscription.productId?.portfolioName || 
                        'Your subscription',
          product_type: existingSubscription.productType,
          invite_link: existingSubscription.invite_link_url,
          expires_at: existingSubscription.invite_link_expires_at
        });
      }
      ...
      return res.status(409).json({ 
        ...
        subscriptionDetails: existingSubscription ? {
          ...
          product: existingSubscription.productId?.name || existingSubscription.productId?.portfolioName,
          expiresAt: existingSubscription.expiresAt,
          status: existingSubscription.status
        } : null,
        telegramInvites: telegramInvites.length > 0 ? telegramInvites : null
      });
    }
    ...
    try {
      if (newSubscriptions && newSubscriptions.length > 0) {
        ...
        // Generate bill number - handle the case if function isn't available
        let billNumber;
        try {
          if (typeof billService.generateBillNumber === 'function') {
            billNumber = await billService.generateBillNumber();
          } else {
            // Fallback if the function isn't available
            const now = new Date();
            billNumber = `INV-${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}-${now.getTime().toString().slice(-8)}`;
            logger.warn('billService.generateBillNumber not available, using fallback', { billNumber });
          }
        } catch (billNumError) {
          logger.error('Failed to generate bill number', { error: billNumError.message });
          billNumber = `INV-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
        }
    ...
    if (allTelegramInvites.length > 0) {
      ...
      // If no Telegram invites were generated, try one more time directly for each subscription
      logger.info('No Telegram invites were generated by handlers, trying direct generation');
      const directInvites = [];
      
      // Initialize the Telegram links array in the response data even before we have results
      responseData.telegramInviteLinks = [];
      
      // Process each subscription in sequence to avoid overloading the Telegram API
      for (const sub of newSubscriptions) {
        try {
          // Make sure we have user email - this is critical for Telegram API
          if (!req.user || !req.user.email) {
            logger.error('Cannot generate Telegram invite: User email is missing', {
          ...
          if (sub.productType === 'Portfolio') {
            product = await Portfolio.findById(sub.productId);
          } else if (sub.productType === 'Bundle') {
            product = await Bundle.findById(sub.productId);
          }
          
          if (product && product.externalId) {
            logger.info(`Found product with externalId ${product.externalId}, generating Telegram invite link`, {
              userEmail: req.user.email,
              productId: product._id,
              externalId: product.externalId
            });
            
            // Try to generate the invite link with retry logic
            ...
            while (retryCount <= maxRetries) {
              try {
                inviteResult = await TelegramService.generateInviteLink(req.user, product, sub);
                if (inviteResult.success) break;
                
                // If not successful but not due to timeout, don't retry
            ...
            if (inviteResult?.success) {
              // Update subscription with invite details
              ...
              directInvites.push({
                ...
                product_name: product.name || product.portfolioName || 'Unnamed Product',
              ...
              // Send email
              try {
                await sendTelegramInviteEmail(req.user, product, inviteResult.invite_link, inviteResult.expires_at);
              } catch (emailError) {
                logger.error(`Failed to send Telegram invite email: ${emailError.message}`);
                // Don't fail the entire process if email sending fails
              }
              
              logger.info(`Direct Telegram invite generated for ${sub.productType} ${product.name || product.portfolioName || 'Unnamed Product'}`);
            } else {
              logger.warn(`Failed to generate direct Telegram invite for ${sub.productType} ${product.name || product.portfolioName || 'Unnamed Product'}: ${inviteResult?.error || 'Unknown error'}`);
            }
          } else {
            logger.warn(`Product has no externalId, can't generate Telegram invite`, {
              productId: sub.productId,
              productType: sub.productType
            });
          }
        } catch (err) {
          logger.error(`Error generating direct Telegram invite for subscription ${sub._id}: ${err.message}`);
        }
      }
      
      if (directInvites.length > 0) {
        responseData.telegramInviteLinks = directInvites;
        responseData.telegramMessage = `You have access to ${directInvites.length} Telegram group${directInvites.length > 1 ? 's' : ''}. Check your email for invite links.`;
      }
    }
    ...
    // Ensure all subscription Telegram invites are included in the response
    if (!responseData.telegramInviteLinks || responseData.telegramInviteLinks.length === 0) {
      // Double-check subscriptions directly for invite links
      const subscriptionsWithInvites = await Subscription.find({
        _id: { $in: newSubscriptions.map(sub => sub._id) },
        invite_link_url: { $exists: true, $ne: null }
      }).populate({
        path: 'productId',
        select: 'name portfolioName'
      });
      
      if (subscriptionsWithInvites && subscriptionsWithInvites.length > 0) {
        const extractedInvites = subscriptionsWithInvites.map(sub => ({
          subscriptionId: sub._id,
          productId: sub.productId?._id || sub.productId,
          product_name: sub.productId?.name || sub.productId?.portfolioName || 'Your subscription',
          product_type: sub.productType,
          invite_link: sub.invite_link_url,
          expires_at: sub.invite_link_expires_at
        }));
        
        responseData.telegramInviteLinks = extractedInvites;
        responseData.telegramMessage = `You have access to ${extractedInvites.length} Telegram group${extractedInvites.length > 1 ? 's' : ''}. Check your email for invite links.`;
        logger.info(`Found ${extractedInvites.length} existing Telegram invites from subscriptions`);
      }
    }
    ...
    logger.info('Payment verification completed successfully with coupon support', {
      ...
      isCartCheckout: notes.cartCheckout === "true",
      subscriptionsCreated: newSubscriptions.length,
      telegramInvitesCount: responseData.telegramInviteLinks?.length || 0
    });
...
exports.verifyEmandate = async (req, res) => {
  try {
    ...
    if (["authenticated", "active"].includes(status)) {
      ...
      // If no Telegram invites were generated, try one more time directly
      if (telegramInviteLinks.length === 0) {
        logger.info('No Telegram invites were generated by handlers for eMandate, trying direct generation');
        const directInvites = [];
        
        // Get the user details to ensure we have email for Telegram API
        const user = await User.findById(userId);
        
        if (!user || !user.email) {
          logger.error('Cannot generate Telegram invites for eMandate: User email is missing', {
          ...
          for (const sub of existingSubs) {
            try {
              ...
              if (product && product.externalId) {
                logger.info(`Found product with externalId ${product.externalId}, generating Telegram invite link for eMandate`, {
                  userEmail: user.email,
                  productId: product._id,
                  externalId: product.externalId
                });
              const inviteResult = await TelegramService.generateInviteLink(user, product, sub);
              ...
              if (inviteResult.success) {
                ...
                // Send email
                await sendTelegramInviteEmail(user, product, inviteResult.invite_link, inviteResult.expires_at);
                logger.info(`Direct Telegram invite generated for eMandate ${sub.productType} ${product.name}`);
              } else {
                logger.warn(`Failed to generate direct Telegram invite for eMandate ${sub.productType} ${product.name}: ${inviteResult.error}`);
              }
            } else {
              logger.warn(`Product has no externalId, can't generate Telegram invite for eMandate`);
            }
          } catch (err) {
            logger.error(`Error generating direct Telegram invite for eMandate subscription ${sub._id}: ${err.message}`);
          }
        }
        
        if (directInvites.length > 0) {
          telegramInviteLinks = directInvites;
        }
      }
    }
