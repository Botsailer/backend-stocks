...
4 0 obj
<<
/Length ${getBillContentLength(bill)}
>>
stream
BT
/F1 16 Tf
...
0 -20 Td
(${item.description} - ${formatCurrency(item.totalPrice)}) Tj`).join('')}
0 -40 Td
(Subtotal: ${formatCurrency(bill.subtotal)}) Tj
0 -20 Td
(Tax (${bill.taxRate}%): ${formatCurrency(bill.taxAmount)}) Tj
0 -20 Td
...
function getBillContentLength(bill) {
  // Approximate content length calculation
  return 400 + (bill.items.length * 50);
}
